...
logcopter-check:
	GOWORK=off go tool logcopter-gen -area-prefix go-go-golems.ragopt -strip-prefix github.com/go-go-golems/ragopt -check ./pkg/...
...
release:
	...
	GOWORK=off GOPROXY=proxy.golang.org go list -m github.com/go-go-golems/ragopt@$(shell svu current)
...
RAGOPT_BINARY=$(shell which ragopt)
install:
	GOWORK=off go build -o ./dist/ragopt ./cmd/ragopt && \
		cp ./dist/ragopt $(RAGOPT_BINARY)
